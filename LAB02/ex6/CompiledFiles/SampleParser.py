...
class SampleParser ( Parser ):
    ...
    symbolicNames = [ "<INVALID>", "HELLO_ID_EXCL", "WS", "ERROR_CHAR" ]
    ...
    class StartContext(ParserRuleContext):
        ...
        def HELLO_ID_EXCL(self):
            return self.getToken(SampleParser.HELLO_ID_EXCL, 0)

        def EOF(self):
            return self.getToken(SampleParser.EOF, 0)

        def getRuleIndex(self):
            return SampleParser.RULE_start
    ...
    def start(self):
        ...
        localctx = SampleParser.StartContext(self, self._ctx, self.state)
        self.enterRule(localctx, 0, self.RULE_start)
        try:
            self.enterOuterAlt(localctx, 1)
            self.state = 2
            self.match(SampleParser.HELLO_ID_EXCL)
            self.state = 3
